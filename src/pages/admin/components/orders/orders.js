...
class Orders extends Component {
  ...
  getOrders = () => {
    axios.get(`${serverUrl}/api/order`).then((result) => {
  ...
  getByType = (idType) => {
    axios.get(`${serverUrl}/api/product/getByType/` + idType).then((result) => {
      this.setState({ products: result.data });
    });
  };

  deleteProd = (id) => {
    axios.delete(`${serverUrl}/api/product/` + id).then((result) => {
      this.getOrders();
    });
  };
  alertDelete = (id) => {
                ...
                <div className="card-title-right">
                  ...
                  <form>
                    <div className="input-group no-border">
                      <input
                        ref={(input) => (this.searchProd = input)}
                        onKeyDown={this.handleSearchProd}
                      ...
                      {this.state.orders
                        ? this.state.orders.map((data) => (
                            <tr key={data._id}>
                              ...
                              <td>
                                {data.status ? "Đã xác nhận" : "Chưa xác nhận"}{" "}
                              </td>
                              <td>
                              ...
                              <td className="product_control text-right">
                                {/* <a
                                  href="#repair"
                                  onClick={() => this.sendIdtoModal(prd._id)}>
                                  <i className="fas fa-pen"></i>
                                </a>
                                <a
                                  href="#delete"
                                  onClick={() => this.alertDelete(prd._id)}>
